...
async function createDoctorReviewCommission(applicationId: string, doctorId: string) {
  try {
    const commSettings = await storage.getCommissionSettings();
    const fee = commSettings?.doctorReviewFee;
    if (!fee || !Number.isFinite(fee) || fee <= 0) return;

    const docId = `doctor_review_${applicationId}`;
    const existing = await storage.getCommission(docId);
    if (existing) return;

    await storage.createCommissionWithId(docId, {
      agentId: doctorId,
      applicationId,
      amount: (fee / 100).toFixed(2),
      status: "pending",
      commissionType: "doctor_review",
    } as any);
  } catch (error) {
    console.error("Error creating doctor review commission:", error);
  }
}

async function createReferralCommission(applicationId: string, applicantUserId: string) {
  try {
    const commSettings = await storage.getCommissionSettings();
    const payout = commSettings?.referralPayoutAmount;
    if (!payout || !Number.isFinite(payout) || payout <= 0) return;
    ...
    await storage.createCommissionWithId(docId, {
      agentId: applicant.referredByUserId,
      applicationId,
      referredUserId: applicantUserId,
      amount: (payout / 100).toFixed(2),
      status: "pending",
...
export async function registerRoutes(
  ...
  app.post("/api/upload/pet-photo", requireAuth, (req, res, next) => {
    memoryUpload.single("petPhoto")(req, res, async (err) => {
      if (err) {
        res.status(400).json({ message: err.message });
        return;
      }
      ...
      try {
        const bucket = firebaseStorage.bucket();
        const fileName = `pet-photos/${req.user!.id}_${Date.now()}_${req.file.originalname.replace(/[^a-zA-Z0-9._-]/g, "_")}`;
        const file = bucket.file(fileName);
        await file.save(req.file.buffer, {
          metadata: { contentType: req.file.mimetype },
        });
        ...
        res.status(500).json({ message: "Failed to upload pet photo" });
      }
    });
  });
    ...
    try {
      ...
      if (doctor) {
        ...
        if (adminSettings?.autoCompleteApplications) {
          ...
          await createDoctorReviewCommission(application.id, doctor.userId || doctor.id);
  ...
  app.post("/api/applications", requireAuth, async (req, res) => {
    try {
      ...
      const registrationId = generateRegistrationId();
      const effectivePaymentStatus = reqPaymentStatus || "unpaid";
      const applicationStatus = (effectivePaymentStatus === "unpaid" && Number(pkg.price) > 0) ? "awaiting_payment" : "pending";
      const application = await storage.createApplication({
        userId: req.user!.id,
        packageId,
        currentStep: 1,
        ...
        try {
          ...
          if (doctor) {
            ...
            if (adminSettings?.autoCompleteApplications) {
              ...
              fireAutoMessageTriggers(application.id, "doctor_approved");
              await createDoctorReviewCommission(application.id, doctor.userId || doctor.id);
              application.status = "doctor_approved" as any;
  ...
  app.post("/api/admin/users/:userId/manual-payment", requireAuth, requireLevel(3), async (req, res) => {
    try {
      ...
      if (targetUser.userLevel !== 1) {
        res.status(400).json({ message: "Manual payment is only available for Applicant-level users" });
        return;
      }
      if (!targetUser.isActive) {
        res.status(400).json({ message: "Cannot process payment for an inactive user" });
        return;
      }
      const pkg = await storage.getPackage(packageId);
      if (!pkg) {
      ...
      if (!pkg.isActive) {
        res.status(400).json({ message: "Cannot use an inactive package" });
        return;
      }
      const rawSteps = pkg.workflowSteps as string[] | undefined;
    ...
    try {
      ...
      if (doctor) {
        ...
        if (adminSettings?.autoCompleteApplications) {
          ...
          await createDoctorReviewCommission(applicationId, doctor.userId || doctor.id);
  ...
  app.post("/api/review/:token/decision", async (req, res) => {
    try {
      ...
      if (decision === "approved") {
        ...
        await createDoctorReviewCommission(tokenRecord.applicationId, tokenRecord.doctorId);
  ...
  app.post("/api/admin/verification-queue/:id/verify", requireAuth, requireLevel(3), async (req, res) => {
    try {
      ...
      if (approved) {
        ...
        if (app.userId) {
          await createReferralCommission(req.params.id as string, app.userId);
        }
        res.json(updated);
      } else {
        const updated = await storage.updateApplication(req.params.id as string, {
  ...
  app.get("/api/admin/commission-settings", requireAuth, requireLevel(3), async (req, res) => {
    try {
  ...
  app.put("/api/admin/commission-settings", requireAuth, requireLevel(3), async (req, res) => {
    try {
      const { doctorReviewFee, referralPayoutAmount } = req.body;
      if (typeof doctorReviewFee !== "number" || !Number.isFinite(doctorReviewFee) || doctorReviewFee < 0) {
        res.status(400).json({ message: "doctorReviewFee must be a non-negative number (in cents)" });
        return;
      }
      if (typeof referralPayoutAmount !== "number" || !Number.isFinite(referralPayoutAmount) || referralPayoutAmount < 0) {
        res.status(400).json({ message: "referralPayoutAmount must be a non-negative number (in cents)" });
        return;
      }
      const existing = await storage.getCommissionSettings() || {};
      const updated = await storage.updateCommissionSettings({
        ...existing,
        doctorReviewFee: Math.round(doctorReviewFee),
        referralPayoutAmount: Math.round(referralPayoutAmount),
      });
      res.json(updated);
    } catch (error: any) {
      res.status(500).json({ message: error.message });
  ...
  app.post("/api/forms/fill-letter", requireAuth, async (req, res) => {
    try {
      ...
      for (const [placeholder, value] of Object.entries(placeholderMap)) {
        while (rawStr.includes(placeholder)) {
          rawStr = rawStr.replace(placeholder, value);
        }
      }
